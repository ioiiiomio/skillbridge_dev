...
const accentStyles: Record<GameItem["accent"], { bg: string; icon: string; ring: string }> = {
  blue: {
...
export function GameCard({ game }: GameCardProps) {
  ...
  return (
    <div
      className={cn(
        "group flex h-full flex-col rounded-2xl border border-border p-7 transition-all duration-300 hover:-translate-y-1",
        style.bg,
        style.ring
      )}
    >
      ...
      <p className="mt-2.5 flex-1 text-[15px] leading-relaxed text-ink/60">
        {game.description}
      </p>
      ...
      <button
        type="button"
        className="mt-5 inline-flex items-center gap-1.5 self-start rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-primary shadow-card transition-transform group-hover:-translate-y-0.5"
      >
        {game.buttonLabel}
